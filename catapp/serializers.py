...
class HumanSerializer(HyperlinkedModelSerializer):
    cats = serializers.HyperlinkedRelatedField(many=True, read_only=True, view_name='cat-detail')
...
class CatSerializer(HyperlinkedModelSerializer):
    ...
    class Meta:
        model = Cat
        fields = ('url', 'name', 'gender', 'date_of_birth', 'description', 'breed', 'owner', 'home')